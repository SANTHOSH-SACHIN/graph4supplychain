...
#     return model, best_test_accuracy, best_test_loss
...
#         # Print epoch metrics
#         st.write(f"Epoch {epoch + 1}/{num_epochs}")
#         st.write(f"Train Loss: {avg_train_loss:.4f}, Train MSE: {avg_train_mse:.4f}, Train MAE: {avg_train_mae:.4f}")
#         st.write(f"Test Loss: {avg_test_loss:.4f}, Test MSE: {avg_test_mse:.4f}, Test MAE: {avg_test_mae:.4f}, Test R²: {avg_test_r2:.4f}")
#         st.write("----------------------------------------")
    
#     # Load best model
#     if best_state:
#         model.load_state_dict(best_state)
#     return model, best_test_r2, best_test_mae
...
#             local_dir = st.sidebar.text_input("Enter local directory path", "./")
...
#             epochs = st.sidebar.slider("Number of Epochs", 50, 200, 100)
...
#                 base_url = "https://cow-upright-mostly.ngrok-free.app/"
#                 version = "NSS_1000_12"
#                 headers = {"accept": "application/json"}
...
#                 # Start training
#                 if task_type == "Classification":
#                     trained_model, best_test_accuracy, best_test_loss = train_classification(
#                         num_epochs, model, optimizer, loss_fn, temporal_graphs, label='PARTS', device=device, patience=patience
#                     )
#                 else:
#                     trained_model, best_test_r2, best_test_mae = train_regression(
#                         num_epochs, model, optimizer, loss_fn, temporal_graphs, label='PARTS', device=device, patience=patience
#                     )
...
#                     fig = go.Figure()
#                     fig.add_trace(go.Bar(x=['Accuracy', 'Loss'], y=[best_test_accuracy, best_test_loss]))
#                     fig.update_layout(title="Model Performance", xaxis_title="Metrics", yaxis_title="Values")
#                     st.plotly_chart(fig)
#                 else:
#                     col1, col2 = st.columns(2)
...
#                     # Plot performance graph
#                     fig = go.Figure()
#                     fig.add_trace(go.Bar(x=['R²', 'MAE'], y=[best_test_r2, best_test_mae]))
#                     fig.update_layout(title="Model Performance", xaxis_title="Metrics", yaxis_title="Values")
#                     st.plotly_chart(fig)
